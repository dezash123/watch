...
pub struct FrameBufferIcna5300<'a, const X: usize, const Y: usize, SPI: SpiDevice, TE: Wait, RST: OutputPin, RGB: PixelColor> {
    framebuffer_1: [[RGB; X]; Y],
    framebuffer_2: [[RGB; X]; Y],
    current_buffer: &'a [[RGB]],
    _buffer: &'a [[RGB]],
    spi: SPI,
    tearing_enable: TE,
    reset: RST,
...
impl<SPI: SpiDevice, TE: Wait, RST: OutputPin, RGB: PixelColor> FrameBufferIcna5300<SPI, TE, RST, RGB>
where
