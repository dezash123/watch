...
static unsigned int g_upgrade_flag =
    0; /* 0:driver init upgrade, 1:OTA upgrade */
...
static unsigned char cmd_2dma_42bd[6] = {
    /*0x42, 0xbd, */ 0x28, 0x35, 0xc1, 0x00, 0x35, 0xae};

/* RETURN:0->pass else->fail */
static int chsc6x_read_bytes_u16addr(unsigned char id, unsigned short adr,
                                     unsigned char *rxbuf,
  ...
  while (len > 0) {
    ...
    retry = 0;
    while (chsc6x_read_bytes_u16addr_sub(id, ofs_adr, &rxbuf[offset], rd_len) !=
           0) {
      if (retry++ == 3) {
        ret = -1;
        break;
...
/* RETURN:0->pass else->fail */
static int chsc6x_write_bytes_u16addr(unsigned char id, unsigned short adr,
                                      unsigned char *rxbuf,
                                      unsigned short lenth) {
  int ret = 0;
  int retry;
  ...
  while (len > 0) {
    ...
    retry = 0;
    while (chsc6x_write_bytes_u16addr_sub(id, ofs_adr, &rxbuf[offset],
                                          wr_len) != 0) {
      if (retry++ == 3) {
        ret = -1;
        break;
...
static int chsc6x_bulk_down_check(unsigned char *pbuf, unsigned short addr,
                                  unsigned short len) {
...
static unsigned short chsc6x_checksum_u16(unsigned short *buf,
...
static unsigned int chsc6x_checksumEx(unsigned char *buf,
                                      unsigned short length) {
...
static int chsc6x_cmd_send(ctp_tst_wr_t *ptchcw, ctp_tst_rd_t *pcr) {
  ...
  ptchcw->tag = 0x35;
  ptchcw->chk =
      1 + ~(chsc6x_checksum_u16((unsigned short *)ptchcw, LEN_CMD_CHK_TX));
  ptchcw->tag = 0x30;
  ret = chsc6x_write_bytes_u16addr(g_i2c_addr, CMD_ADDR,
                                   (unsigned char *)ptchcw, LEN_CMD_PKG_TX);
  if (ret) {
    goto exit;
  }
  ptchcw->tag = 0x35;
  ret = chsc6x_write_bytes_u16addr(g_i2c_addr, CMD_ADDR + 9,
                                   (unsigned char *)&(ptchcw->tag), 1);
  if (ret) {
    goto exit;
  }
  ...
  while (retry++ < 100) { /* 2s */
    chsc6x_msleep(20);
    if (chsc6x_read_bytes_u16addr(g_i2c_addr, RSP_ADDR, (unsigned char *)pcr,
                                  1)) {
      break;
    }
    ...
    /* chsc6x_msleep(50); */
    chsc6x_read_bytes_u16addr(g_i2c_addr, RSP_ADDR, (unsigned char *)pcr,
                              LEN_RSP_CHK_RX);
...
/* return 0=successful */
static int chsc6x_read_burn_space(unsigned char *pdes, unsigned short adr,
                                  unsigned short len) {
  int rsp = -1;
...
static int chsc6x_write_burn_space(unsigned char *psrc, unsigned short adr,
                                   unsigned short len) {
  int rsp = 0;
...
static int chsc6x_get_running_cfg(unsigned short *ptcfg, unsigned short addr) {
  ...
  while (++retry < 5) {
    err_type = 0;
    if (chsc6x_read_bytes_u16addr(g_i2c_addr, addr, (unsigned char *)ptcfg,
                                  204)) {
      chsc6x_msleep(20);
...
static int chsc6x_find_ver(void) {
  ...
  if (is_valid_cfg_data(buf_tmpcfg)) {
    ...
    g_pfw_infos->chsc6x_vendor_id = ((g_chsc6x_cfg_ver >> 9) & 0x7F |
                                     ((g_chsc6x_cfg_ver >> 22 & 0x03) << 7));
    g_pfw_infos->chsc6x_project_id =
        ((g_chsc6x_cfg_ver & 0x01FF) | ((g_chsc6x_cfg_ver >> 20 & 0x03) << 9));
  }
...
/*return: 0 SUCESS else FAIL*/
static int chsc6x_do_update_ifneed(unsigned char *p_fw_upd,
                                   unsigned int fw_len) {
  ...
  if (41 == g_pfw_infos->chsc6x_vendor_id &&
      27 == g_pfw_infos->chsc6x_project_id) {
    fupd = chsc_boot_41_27;
    fw_size = sizeof(chsc_boot);
    ret = chsc6x_update_compat_ctl((unsigned char *)fupd, fw_size);
  } else if (29 == g_pfw_infos->chsc6x_vendor_id &&
             08 == g_pfw_infos->chsc6x_project_id) {
...
void chsc6x_get_chip_info(struct ts_fw_infos *infos) {
  ...
  for (cnt = 0; cnt < 3; cnt++) {
    ...
    if ((ret != 0) || (data_t[0] == 0x0) || (data_t[2] == 0x0)) {
      ...
      infos->chsc6x_project_id =
          (((data_t[1] & 1) << 8) | data_t[0]) | ((data_t[2] & 0x30) << 5);
      chsc6x_info(
          "chsc6x: TP chip_info OK::fw_ver:%d, vendor_id:%d, prj_id:%d \r\n",
          infos->chsc6x_cfg_version, infos->chsc6x_vendor_id,
          infos->chsc6x_project_id);
    }
    ...
    if (0 == ret) {
      ...
      chsc6x_info(
          "chsc6x: chsc6x_chip_id & chsc6x_chip_type OK::0x%02x:0x%x \r\n",
...
*/
int chsc6x_tp_dect(struct ts_fw_infos *pfw_infos,
  ...
  if (chsc6x_get_running_cfg(buf_tmpcfg, 0x9e00) == 0) {
    ...
    chsc6x_info(
        ...
        g_pfw_infos->chsc6x_boot_version, g_pfw_infos->chsc6x_cfg_version,
        g_pfw_infos->chsc6x_chip_id);
  } else {
...
OTA.
/* PARM pfw_infos: to get top 6 fw infos in struct ts_fw_infos, after ota
upgrade.
/* PARM p_fw_upd: array address of the upgrade firmware array
/* PARM fw_len: total size of the upgrade firmware array
/* RETURN NULL
*/
void chsc6x_ota_upgrade_tp_fw(struct ts_fw_infos *pfw_infos,
                              unsigned char *p_fw_upd, unsigned int fw_len) {
  int ret;
  int try_cnt;
  unsigned char dwr = 0x05;
